...
const ReviewedGiftCard: React.FC<ReviewedGiftCardProps> = ({
  ...
  const reviewedProducts = singleGift.productDetails.filter((productDetail) => {
    return singleGift.productReviews.some(
      (review) => review.productId === productDetail._id
    );
  });

  // Get review for each product
  const getProductReview = (productId: string) => {
    return singleGift.productReviews.find(
  ...
  return (
    <div className="border rounded-lg mb-6 overflow-hidden">
      <div className="bg-gray-100 p-4 flex justify-between items-center">
        ...
        <div
          className={`rounded-3xl px-3 py-1 ${getStatusColor(
      ...
      {reviewedProducts.map((product) => {
        ...
        return (
          <div
            key={product._id}
            className="card card-side bg-base-100 shadow-xl rounded-none border-t-2 p-5"
          >
            {/* Product Image */}
              ...
              {review && (
                <>
                  ...
                  <div className="mt-4">
                    <button
                      onClick={() =>
                        handleReviewClick(singleGift.tran_id, product._id)
                      }
                      disabled={!isDelivered}
                      className={`btn btn-sm text-xs whitespace-nowrap border-primary text-primary hover:text-white hover:bg-primary  ${
